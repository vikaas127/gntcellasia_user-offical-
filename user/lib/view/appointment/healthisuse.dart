...
class Healthisse extends StatefulWidget {
  @override
  HealthisseState createState() => HealthisseState();
}

class HealthisseState extends State<Healthisse> {
  List<Healthissuedata> healthissuelist = [];
  bool _loadding = false;
  String? _Address = "";
  TextEditingController _search = TextEditingController();
  List<Healthissuedata> _searchResult = [];
        ...
        body: GestureDetector(
          ...
          child: _searchResult.length > 0 || _search.text.isNotEmpty
            ...
            child: Text(
              ...
              : healthissuelist.length != 0
              ? SingleChildScrollView(
            child: ListView.builder(
              itemCount: healthissuelist.length,
              shrinkWrap: true,
              scrollDirection: Axis.vertical,
              itemBuilder: (context, index) {
                return Column(
                ...
                child:
                              Container(
                                ...
                                height: 60,
                                alignment: AlignmentDirectional.center,
                                child:  healthissuelist[index].primaryImage!=null?CachedNetworkImage(height: 60,width: 60,
                                  alignment: Alignment.center,
                                  imageUrl: '${Apis.baseUrlImages}${healthissuelist[index].primaryImage!}',
                                  fit: BoxFit.fitHeight,
                              ...
                              Container(
                                margin: EdgeInsets.symmetric(horizontal: width * 0.05),
                                child: Text(
                                  healthissuelist[index].name!,textAlign: TextAlign.center,
                                  style: TextStyle(fontWeight: FontWeight.bold,
                                    fontSize: width * 0.055,
                                    color: Palette.dark_grey,
                                  ),
  ...
  Future<BaseModel<HealthIssue>> callApi() async {
    ...
    try {
      ...
      setState(() {
        if (response.status == 200) {
          setState(() {
            _loadding = false;
            healthissuelist.addAll(response.healthissuedata!);
          });
        }
      });
  ...
  onSearchTextChanged(String text) async {
    ...
    healthissuelist.forEach((appointmentData) {
